...
$$\textbf{Theorem 6.11} (Riesz representation). \textit{Let $H$ be a Hilbert space. For every bounded linear operator $G \in H^*$, there exists unique $y \in H$ such that $G = F_y$. That is, for all $x \in H$, we have}
\[
G(x) = \langle x, y \rangle = F_y(x)
\]
\textit{and} $\|G\|_{H^*} = \|F_y\|_{H^*} = \|y\|$.$$
